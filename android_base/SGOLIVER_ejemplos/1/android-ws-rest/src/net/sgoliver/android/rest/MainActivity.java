...
public class MainActivity extends Activity {
	...
	$sql = "INSERT INTO usuarios (numero, valor, operador, maquina) VALUES "  . "('$numero', '$valor',
								  '$operador', '$maquina'  )";
	    ...
	    protected Boolean doInBackground(String... params) {
	    	...
	    	//httpClient para realizar la comunicacion HTTP con el server
	    	HttpClient httpClient = new DefaultHttpClient();
	        
			HttpPost post = new HttpPost("http://10.0.2.2:2731/Api/Clientes/Cliente");
			//indicar el formato a usar en la comunicacion
			...
			try
	        {
				...
				//asociar el JSONObject con la peticion http
				StringEntity entity = new StringEntity(dato.toString());
				post.setEntity(entity);
					
				//se realiza la llamada al servicio y recuperamos la respuesta	
	        	HttpResponse resp = httpClient.execute(post);
	        	String respStr = EntityUtils.toString(resp.getEntity());
	        	
	        	//el método de inserción devuelve únicamente un valor booleano 
	        	//indicando si el registro se ha insertado correctamente en la base de datos,
	        	if(!respStr.equals("true"))
	        		resul = false;
	        }
	        catch(Exception ex)
